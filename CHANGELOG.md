...
### Added
- **Differential testing against a verified oracle** (`Machine.WriteConvergenceTables`): emit a built machine's step tables in the plain-text format consumed by the machine-checked checker in `normalization-confluence/coq/extraction` (extracted from an axiom-free Coq proof). The checker re-certifies, independently of this Go code, that the tables converge (per-event step functions commute and stay in range), so a bug in gsm's own verification cannot make a non-convergent machine pass the oracle. Only valid states are emitted, remapped to a compact index. `oracle_test.go` runs the cross-check when `GSM_CONVERGENCE_CHECKER` points at the built checker.
