...
**Multi-source (beyond the paper)**: when a target has two independent sources (Remark 8.15) the authority argument no longer picks a winner. gsm supports this by letting the target declare a `Resolver` that deterministically merges its sources, relaxing the tree requirement to an acyclic DAG. The paper leaves this open because the merge is domain logic, not math — so gsm does not claim the paper's theorem here. Instead `Build` **exhaustively verifies**, for the specific federation, that the resolver writes only shared variables, preserves target validity over every reachable source combination, and depends only on the sources; determinism (a pure function of a name-keyed source map) gives order-independence. This is the same verify-don't-assume discipline gsm applies to single-registry CC. A multi-source target without a resolver, or a resolver that could diverge, is rejected.
